...
describe('Noteful API - Notes', function () {
      ...
      return Promise.all([
        ...
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            ...
            expect(item).to.include.all.keys('id', 'userId', 'title', 'content', 'updatedAt', 'createdAt');
    ...
    it('should return correct search results for content search', function () {
      ...
      return Promise.all([dbPromise, apiPromise])
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            expect(item).to.be.a('object');
            expect(item).to.include.all.keys('id', 'userId', 'title', 'createdAt', 'content', 'updatedAt', 'tags'); // Note: folderId and content are optional
            expect(item.id).to.equal(data[i].id);
  ...
  describe('POST /api/notes', function () {
    ...
    it('should create and return a new item when provided valid title (content optional)', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.have.all.keys('id', 'userId', 'title', 'createdAt', 'updatedAt', 'tags');
    ...
    it('should create and return when folderId is an empty string', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.include.keys('id', 'userId', 'title', 'createdAt', 'updatedAt', 'tags');
          return Note.findOne({ _id: res.body.id });
        })
        .then(data => {
          expect(res.body.id).to.equal(data.id);
    ...
    it('should return an error when the tags `id` is not valid ', function () {
      const newItem = {
        title: 'What about dogs?!',
        content: 'Lorem ipsum dolor sit amet, sed do eiusmod tempor...',
        tags: ['NOT-A-VALID-ID']
